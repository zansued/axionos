    ...
    <Card className="border-border/30 bg-card/60">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2">
          <AlertTriangle className="h-4 w-4 text-orange-500" /> Riscos e Alertas
        </CardTitle>
      </CardHeader>
        ...
        {alerts.map((alert) => (
          <div key={alert.id} className="rounded-lg border border-border/30 p-3 space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-muted-foreground">{format(new Date(alert.detectedAt), "dd/MM HH:mm")}</span>
            </div>
            <p className="text-xs text-foreground">{alert.summary}</p>
            <div className="flex items-center justify-between text-[11px]">
              <span className="text-muted-foreground">Subsistema: {alert.affectedSubsystem}</span>
            </div>
            <div className="rounded bg-muted/30 px-2 py-1.5">
              <span className="text-[10px] text-muted-foreground">Recomendação: </span>
