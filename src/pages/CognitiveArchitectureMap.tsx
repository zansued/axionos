  ...
  return useQuery({
    ...
    queryFn: async (): Promise<LayerMetrics> => {
      ...
      const [
        ...
        supabase.from("canon_entries").select("id, confidence_score, approval_status", { count: "exact" }).eq("organization_id", orgId),
        supabase.from("engineering_skills").select("id, lifecycle_status", { count: "exact" }).eq("organization_id", orgId),
        supabase.from("skill_capabilities").select("id, engineering_skill_id", { count: "exact" }).eq("organization_id", orgId),
        supabase.from("agents").select("id, status", { count: "exact" }).eq("organization_id", orgId),
        supabase.from("story_subtasks").select("id, status", { count: "exact" }).eq("organization_id", orgId).gte("created_at", new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()),
        supabase.from("agent_outputs").select("id, status", { count: "exact" }).eq("organization_id", orgId),
        supabase.from("skill_bundles").select("id", { count: "exact" }).eq("organization_id", orgId),
      ]);
