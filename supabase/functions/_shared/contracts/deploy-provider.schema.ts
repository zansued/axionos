...
export class VercelAdapter implements IDeployProviderAdapter {
  ...
  async deploy(initiative: DeployInitiativeContext, config: DeployProviderConfig): Promise<DeployProviderResult> {
    ...
    if (!vercelToken) {
      const deployLink = `https://vercel.com/new/clone?repository-url=${encodeURIComponent(`https://github.com/${owner}/${repo}`)}&project-name=${encodeURIComponent(repo)}`;
      return { deploy_url: deployLink, health_status: "unknown", error_code: null, error_message: null, provider_metadata: { mode: "manual_link" } };
    }
    ...
    try {
      ...
      const getRes = await fetch(`https://api.vercel.com/v9/projects/${encodeURIComponent(projectName)}`, {
      ...
      if (getRes.ok) {
        ...
        const createRes = await fetch("https://api.vercel.com/v10/projects", {
          ...
          body: JSON.stringify({
            name: projectName, framework: config.framework,
            gitRepository: { type: "github", repo: `${owner}/${repo}` },
            installCommand: config.install_command, buildCommand: config.build_command,
      ...
      const health = url ? await this.healthCheck(url) : "unknown";
      return { deploy_url: url, health_status: health, error_code: null, error_message: null, provider_metadata: { deployment_id: deployment.id } };
    } catch (err: any) {
  ...
  parseError(errorCode: string, errorMessage: string): DeployErrorSuggestion[] {
    ...
    const patterns: Array<{ pattern: RegExp; suggestion: DeployErrorSuggestion }> = [
      ...
      { pattern: /PROJECT_CREATE_FAILED/i, suggestion: { error_pattern: "PROJECT_CREATE_FAILED", title: "Project creation failed", suggestion: "Check Vercel token permissions and GitHub repo access", doc_url: "https://vercel.com/docs/rest-api#endpoints/projects/create-a-project", severity: "error" } },
      { pattern: /DEPLOY_FAILED/i, suggestion: { error_pattern: "DEPLOY_FAILED", title: "Deployment failed", suggestion: "Check build logs. Common causes: missing dependencies, build script errors", doc_url: "https://vercel.com/docs/deployments/troubleshoot-a-build", severity: "error" } },
