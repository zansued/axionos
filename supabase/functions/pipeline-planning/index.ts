...
Valide e defina a árvore completa de arquivos:
{
  ...
  "total_files": 0
    ...
    // Clean existing stories for this initiative
    ...
    // Generate PRD and Architecture docs from Layer 1+2 data
    const prdContent = [
    ...
    await updateInitiative(ctx, {
      ...
      discovery_payload: {
        ...
        layer3_total_tokens: taskPlanner.tokens + storyGen.tokens + filePlanner.tokens,
        layer3_total_cost_usd: taskPlanner.costUsd + storyGen.costUsd + filePlanner.costUsd,
      },
    });
    ...
    return jsonResponse({
      success: true, agents_executed: 3, layers_completed: [3],
      stories: createdStories, total_subtasks: totalSubtasks,
      scaffold_files: scaffoldFiles, total_tokens: totalTokens,
      total_cost_usd: totalCost, job_id: jobId,
    });
  } catch (e) {
